...
function startInterview() {
    ...
    const rounds = document.getElementById('questionCount').value;
    const protocol = window.location.protocol === "https:" ? "wss" : "ws";
    ws = new WebSocket(`${protocol}://${window.location.host}/ws/interview?pos=${encodeURIComponent(role)}&rounds=${rounds}`);
    ...
    ws.onmessage = function (event) {
        ...
        if (data.startsWith("SYSTEM_END:")) {
            ...
            // Replace input area with Return to Homepage button
            const inputArea = document.getElementById('input-area');
            inputArea.innerHTML = ''; // Clear existing input and send button

            const homeBtn = document.createElement('button');
            homeBtn.innerText = "Return to Homepage";
            homeBtn.onclick = function () { window.location.reload(); };
            homeBtn.style.width = "100%"; // Make button full width

            inputArea.appendChild(homeBtn);
